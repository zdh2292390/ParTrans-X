...
vector<vector<double> > relation_grad,entity_grad;
...
public:
    ...
    void run(int n_in,double rate_in,double margin_in,int method_in)
    {
        ...
        if(strategy=="adagrad")
        {
            entity_grad.resize(entity_num);
            for (int i=0; i<entity_grad.size(); i++)
                entity_grad[i].resize(n,1);
            relation_grad.resize(relation_num);
            for (int i=0; i<relation_grad.size(); i++)
                relation_grad[i].resize(n,1);
        }
...
private:
    ...
    void bfgs()
    {
        ...
        #pragma omp parallel for firstprivate(ok) //entity_vec relation_vec加在这里没用，因为train_kb里调用的还是entity_vec的全局变量 
    ...
    void ada_gradient(int e1_a,int e2_a,int rel_a,int e1_b,int e2_b,int rel_b)
    {
        for (int ii=0; ii<n; ii++)
        {
            double x = 2*(entity_vec[e2_a][ii]-entity_vec[e1_a][ii]-relation_vec[rel_a][ii]);
            ...
            relation_vec[rel_a][ii]-=-1*rate*x/sqrt(relation_grad[rel_a][ii]);
            entity_vec[e1_a][ii]-=-1*rate*x/sqrt(entity_grad[e1_a][ii]);
            entity_vec[e2_a][ii]+=-1*rate*x/sqrt(entity_grad[e2_a][ii]);
            relation_grad[rel_a][ii]+=x*x;
            entity_grad[e1_a][ii]+=x*x;
            entity_grad[e2_a][ii]+=x*x;
            ...
            relation_vec[rel_b][ii]-=rate*x/sqrt(relation_grad[rel_b][ii]);
            entity_vec[e1_b][ii]-=rate*x/sqrt(entity_grad[e1_b][ii]);
            entity_vec[e2_b][ii]+=rate*x/sqrt(entity_grad[e2_b][ii]);
            relation_grad[rel_b][ii]+=x*x;
            entity_grad[e1_b][ii]+=x*x;
            entity_grad[e2_b][ii]+=x*x;
        }
    }
    void train_kb(int e1_a,int e2_a,int rel_a,int e1_b,int e2_b,int rel_b,double &private_res)
    {
        ...
        if (sum1+margin>sum2)
        {
            ...
            if(strategy=="sgd")
                ...
                ada_gradient(e1_a, e2_a, rel_a, e1_b, e2_b, rel_b);
        }
    }
};
...
{   
    ...
    {
        version = argv[1];
        cout<<"version:"<<version<<" nthread:"<<argv[2]<<" dataset:"<<argv[3]<<" k:"<<argv[4]<<" margin:"<<argv[5]<<" nepoch:"<<argv[6]<<endl;
